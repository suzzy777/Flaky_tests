...
for polluter in $ab;do
	...
	if [ "$cleaner" == "EMPTY" ];then
	   ...
	   #pro
	    ...
	    od_type=$(awk -F, '$4 == "'"$od_test"'" && $6 == "'"$polluter"'" { print $5}' Patchesmd5.csv)
	else
	...
	#	"'"$a"'"
	    ...
	    patch=$(awk -F, '$4 == "'"$od_test"'" && $6 == "'"$polluter"'" && $7 == "'"$cleaner"'" { print $8}' Patchesmd5.csv)
            od_type=$(awk -F, '$4 == "'"$od_test"'" && $6 == "'"$polluter"'" && $7 == "'"$cleaner"'" { print $5}' Patchesmd5.csv)
	...
	echo $od_test;echo $od_type;echo $polluter;echo $cleaner;echo $patch;echo $md5;                                                                                    # $pollu#ter Patchesmd5.csv | grep $cleaner Patchesmd5.csv | cut -d, -f9)
